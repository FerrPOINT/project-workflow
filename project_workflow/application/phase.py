...
class PhaseServiceApp:
    ...
    def create_phase(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        phase_data = {'workflow_id': workflow_id, 'code': data.get('code') or self._generate_code(workflow_id, order), 'name': data.get('name', self.DEFAULT_PHASE_NAME), 'description': data.get('description', ''), 'execution_type': data.get('execution_type', 'sync'), 'phase_order': order, 'agent_id': data.get('agent_id'), 'next_recommendation': data.get('next_recommendation'), 'parallel_with': data.get('parallel_with'), 'rollback_target': data.get('rollback_target'), 'is_seed_managed': data.get('is_seed_managed', False), 'min_time_min': data.get('min_time_min', 0)}
