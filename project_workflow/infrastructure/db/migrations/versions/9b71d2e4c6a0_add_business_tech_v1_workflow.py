...
def _seed() -> list[dict[str, Any]]:
    ...
    historical_raw = (
        ...
        .replace(
            (
                '"checks": ["PR target равен main", "PR head совпадает с веткой run", '
                '"Build запущен", "Business status равен In Progress"],'
            ).encode(),
            (
                '"checks": ["PR target равен main", "PR head совпадает с веткой run", '
                '"Build запущен", "Business status равен In Review"],'
            ).encode(),
        )
        .replace(b"\r\n", b"\n")
    )
