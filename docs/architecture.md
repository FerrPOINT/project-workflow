...
- **CLI** предоставляет только `step` и `history`. Пользовательские команды для
  отдельных entrypoints создаются wrapper-скриптами и внутри вызывают тот же
  `project-workflow step/history`.
- **Web UI** владеет CRUD для workflow, фаз, CLI entrypoints, агентов и задач через
...
Физическая таблица для этих записей пока называется `projects`. Это legacy-слой
хранения, оставленный ради безопасной миграции. Публичный пользовательский слой
показывает реальные названия и CLI-команды; `/api/projects` и `/api/contexts`
остаются только compatibility aliases.

## State And Audit

`tasks` хранит текущий snapshot задачи: статус, выбранную запись, workflow и текущую
фазу. `task_key` уникален внутри одной такой записи, а не глобально по базе. Это
позволяет вести одну внешнюю задачу, например `RUN-42`, через несколько
entrypoints: для разработки, проверки, релиза или любого другого сценария.
...
Когда одна внешняя задача существует через несколько entrypoints, исполнитель
использует configured CLI-команду нужной записи, например
