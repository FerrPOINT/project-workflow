# Plan: smart agent in `wartz-workflow` via ChatGPT/OpenAI subscription

Date: 2026-06-06
Project: `/opt/dev/hermes-workspace/wartz-workflow-cli`
...
- Main UI routes live in `wartz_workflow/ui.py`:
  - `/` — minimal dashboard
  - `/phases`
  - `/phase/{phase_id}`
  - `/tasks`
  ...
  - `/agents`
  - `/settings`
- `settings` is already intentionally **read-only CLI reference**, auto-built from real Click commands by `_load_cli_reference()` in `wartz_workflow/ui.py`.
...
### Existing data model
Current SQLite schema in `wartz_workflow/db_schema.sql` contains:
- `agents`
- `workflows`
- `phases`
- `instructions`
...
- `wartz_workflow/conversation.py` already stores task conversation in a separate SQLite DB (`~/.wartz-workflow/conversation.db`).
- Roles already include `user | system | wizard | agent`.
- `wartz_workflow/wizard.py` already provides:
...
### `wartz_workflow/assistant_service.py`
Responsibilities:
...
### `wartz_workflow/assistant_provider_openai.py`
Responsibilities:
...
### `wartz_workflow/subscription_service.py`
Responsibilities:
...
### `wartz_workflow/context_builder.py` (optional, but clean)
Responsibilities:
- combine:
  - task record
  - project record
...
## Services
- `wartz_workflow/assistant_service.py` (new)
- `wartz_workflow/subscription_service.py` (new)
- `wartz_workflow/assistant_provider_openai.py` (new)
- `wartz_workflow/context_builder.py` (optional new)

## UI/backend routes
- `wartz_workflow/ui.py`
