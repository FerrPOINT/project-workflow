...
class TestWizardFullContext:
    ...
    def test_context_structure(self):
        engine = wizard.WizardEngine("TASK-999", "/tmp")
    ...
    def test_task_key_passed_through(self):
        engine = wizard.WizardEngine("TASK-42", "/tmp")
        ctx = engine.get_full_context()
        assert ctx["task_key"] == "TASK-42"

    def test_repo_passed_through(self):
        engine = wizard.WizardEngine("TASK-42", "/opt/dev/repo")
        ctx = engine.get_full_context()
        assert ctx["repo"] == "/opt/dev/repo"

    def test_all_phases_present(self):
        engine = wizard.WizardEngine("TASK-1", "/tmp")
        ctx = engine.get_full_context()
