...
class TestOutJson:
    def test_ok_true_exits_0(self):
        from project_workflow.cli.core import out_json
...
class TestGetTaskKeyValidator:
    def test_no_projects_fallback(self, monkeypatch):
        from project_workflow.cli.core import _get_task_key_validator
        db = MagicMock()
        db.get_projects.return_value = []
        monkeypatch.setattr("project_workflow.db.WorkflowDB", lambda: db)
...
class TestRequireValidKey:
    def test_valid_returns_normalized(self, monkeypatch):
        from project_workflow.cli.core import _require_valid_key
        monkeypatch.setattr(
