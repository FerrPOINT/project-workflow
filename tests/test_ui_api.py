...
class TestIndex:
    ...
    def test_tasks_page_is_read_only(self, client):
        ...
        assert resp.status_code == 200
        assert 'data-testid="task-start-form"' not in resp.text
        assert "/api/tasks/step" not in resp.text
...
class TestApiTasks:
    ...
    def test_task_step_api_is_not_registered(self, client):
        response = client.post("/api/tasks/step", json={"task": "RUN-905"})
...
class TestPageRoutes:
    ...
    def test_task_detail_page(self, client):
        ...
        assert resp.status_code == 200
        assert 'data-testid="task-runtime-panel"' not in resp.text
        assert 'data-testid="task-runtime-current"' not in resp.text
        assert 'data-testid="task-runtime-report"' not in resp.text
        assert 'data-testid="task-runtime-submit"' not in resp.text
        assert "/api/tasks/step" not in resp.text
        assert "taskRuntimeStep" not in resp.text
