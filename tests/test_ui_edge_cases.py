...
class TestSeedToSqlite:
    def test_calls_ensure_phase_catalog(self, monkeypatch):
        db = MagicMock()
        monkeypatch.setattr("project_workflow.ui._app_state", MagicMock(db=db))
...
class TestLoadWorkflows:
    def test_empty(self, monkeypatch):
        ...
        db.get_phases.return_value = []
        monkeypatch.setattr("project_workflow.ui._app_state", MagicMock(get_db=lambda: db))
...
class TestTaskDetailEdgeCases:
    def test_task_detail_supervisor_runs_next_contract(self, monkeypatch):
        from project_workflow.ui import _get_task_detail
        db = MagicMock()
    ...
    def test_main_entry(self, monkeypatch):
        from project_workflow.ui import main
        called = []
